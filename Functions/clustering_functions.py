...
def get_HPOID_cluster_freq(decipher_cluster_df, all_HPOTerms):
    clusterHPO = decipher_cluster_df["propagated_terms"].tolist()
...
def get_sigincreased_HPOTerms_cluster(cluster_num, clusters_freq, cluster_freqs):
        cluster_index = cluster_num-1
        for index, row in clusters_freq.iterrows():
                cluster_freq = row["cluster_"+str(cluster_num)+"_freq"]
                outside_cluster_freq = (row[1:13].sum(axis=0) - cluster_freq)
